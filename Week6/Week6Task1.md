...
- ```struct page```：页描述符结构，用于描述系统中每一个物理内存页框（通常是4KB大小）。内核会为物理内存中的每一页分配一个 ```struct page``` 实例，因此在一个拥有数百万页的系统中，就存在数百万个 ```struct page``` 结构。通过 ```struct page```，内核可以跟踪每一页的状态（例如是否空闲、属于哪个内存区域或映射到了哪个进程地址空间，是否在缓存链表上，引用计数等）。这些页描述符通常被集中存放在一个全局数组（旧称 mem_map）中，以页帧号（PFN）索引。```struct page``` 是内存管理的基本单元，内核用它来表示和管理物理内存页。

- ```struct vm_area_struct```（VMA）：虚拟内存区域描述符，用于描述进程地址空间中一段连续的虚拟地址区间及其属性。每个 VMA 通常对应进程地址空间中具有相同访问属性的一片区域（例如某个内存映射的文件、一段匿名的堆内存、栈区的一段等）。VMA 记录了该区域的起始和结束虚拟地址、访问权限（读写执行标志）、映射的文件（如果有）以及偏移量等信息。当用户进程调用 ```mmap()``` 分配内存时，内核就创建一个新的 ```vm_area_struct``` 来代表该映射区域。内核将每个进程的所有 VMA 按照起始地址有序地组织起来（Linux 6.x 内核中使用基于 Maple Tree 的高效数据结构来管理 VMA 列表）。可以通过查看 ```/proc/<pid>/maps``` 来直观地看到一个进程的各个虚拟内存区域及其属性，每一行通常对应一个 VMA。总之，VMA 列出了进程已用到的所有虚拟地址范围，对应着进程的内存布局，例如代码段、数据段、堆、栈、内存映射文件等。

- ```struct mm_struct```：内存描述符，每个进程（线程组）都有一个 ```mm_struct``` 实例，描述该进程完整的虚拟地址空间信息。```mm_struct``` 中包含了指向该进程所有 VMA 的数据结构（例如红黑树或链表的根），还维护了页表的指针（如 pgd 基址）、内存使用统计（例如数据段大小、总的虚拟内存页数等）以及内存管理所需的各种锁（如管理 VMA 的读写信号量 ```mmap_lock``` 等）。在进程的 ```task_struct``` 中有一个指向 ```mm_struct``` 的指针（即 ```task_struct->mm```），用来关联到该进程的内存空间。对线程（轻量级进程）来说，它们共享同一个 ```mm_struct```（在线程的 ```task_struct```中，```mm``` 指针相同），这就是为什么线程共享进程的地址空间。```mm_struct``` 可以看作是进程内存空间的“总账本”，里面的内容包括该进程的所有 VMA 列表/树、页面表、以及内存管理相关的统计和元数据。
